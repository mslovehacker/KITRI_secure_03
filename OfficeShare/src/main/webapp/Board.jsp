...
<html style="font-size: 16px;">
  <head>
	...
	<link rel="stylesheet" href="JSP/Board.css" media="screen">
  ...
  <body class="u-body"><header class="u-clearfix u-gradient u-header u-header" id="sec-8385">
    ...
    <section class="u-align-center u-clearfix u-section-1" id="sec-34a0">
      <div class="u-clearfix u-sheet u-sheet-1">
        <h1 class="u-align-left u-text u-title u-text-1">질문 게시판</h1>
        <div class="u-expanded-width u-table u-table-responsive u-table-1">
           <table border=1 width=700>
	
	<tr>
		<td> 글번호 </td> <td> 글제목 </td> <td> 작성자 ID </td> <td> 비밀글여부 </td> <td> 조회수 </td>
	</tr>
	...
	<c:forEach items="${LIST}" var="ls">
		<tr>
			<td>${ls.id}</td>
			<td><a href="board.do?id=${ls.id}">${ls.subject}</a><c:if test="${ls.newyn lt 2}"><img src="images/newGIF.gif" height="20" width="40"></c:if></td>
			<td>${ls.userId}</td>
			<td>
			<c:choose>
				<c:when test="${ls.protect eq 'true'}">비공개</c:when>
				<c:when test="${ls.protect eq 'false'}">공개</c:when>
			</c:choose>
			</td>
			<td>${ls.hit}</td>
		</tr>	
        ...
        <a href="Boardpage.jsp" class="u-active-custom-color-7 u-border-2 u-border-black u-btn u-btn-round u-button-style u-hover-palette-1-light-1 u-palette-1-base u-radius-3 u-text-body-alt-color u-text-hover-white u-btn-1">글 작성</a>
      </div>
    </section>
